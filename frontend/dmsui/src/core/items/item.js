...
const apiLocationEndPoints = API_END_POINTS.INVENTORY_LOCATIONS_END_POINT;
const apiItemTypesEndPoints = API_END_POINTS.INVENTORY_ITEM_TYPES_END_POINT;
const apiItemMakersEndPoints = API_END_POINTS.INVENTORY_ITEM_MAKERS_END_POINT;
...
class Item extends React.Component {
  ...
  fetchAndPopulateOne(url) {
    RefDataHub.fetchInventoryItem(this.getObjectRefId())
      .then(received => {
  ...
  getHeaderLabel() {
    if (this.isNewItem()) {
      return "Add Asset  " + this.state.savingMsg;
    }
    else {
      ...
      return "Edit Asset :" + itemId + "  " + this.state.savingMsg;
    }
      ...
      <div align="left">
        ...
        <form className={classes.root} noValidate autoComplete="off">
          ...
          <div>
            ...
            <Button style={{ textAlign: 'center', margin: 10 }} variant="dark" onClick={(event) => this.props.history.push(ROUTE_LINKS.ALL_INVENTORY_ITEMS_LINK)}>
              Show All {' '}
              <FontAwesomeIcon icon={faSave} size="1x" color='white' speed="6x" />
            </Button>
